...
static int eyaffs_mount    ( cyg_fstab_entry *fste, cyg_mtab_entry *mte )
{
    ...
    while (m != &cyg_mtab_end) {
        if (m->valid
                ...
                && (yaffs_DeviceToContext(m->data) == part) ) {
            ER(EBUSY);
    ...
    // At the time of writing, the options field is not present in eCos.
#else
    if (mte->options) {
        // If you get a build failure here along the lines of:
        //     ‘cyg_mtab_entry’ has no member named ‘options’
        // then you probably need to turn off CYGSEM_FS_YAFFS_COMPATIBILITY_ECOSPRO.
        int t;
        char buf[10];
    ...
    y->param.name = mte->devname;
    y->context = (void*) part;

    y->nDataBytesPerChunk =
        y->param.totalBytesPerChunk = CYG_NAND_BYTES_PER_PAGE(nand);
    y->param.inbandTags = 0;

    y->param.nChunksPerBlock = CYG_NAND_PAGES_PER_BLOCK(nand);
    y->param.nReservedBlocks = o_nReserved;
    y->param.startBlock = 0;
    y->param.endBlock = CYG_NAND_PARTITION_NBLOCKS(part)-1;
    y->param.useNANDECC = 1;
    y->param.nShortOpCaches = o_nShortOpCaches;
    ...
    y->param.eraseBlockInNAND = eyaffs_eraseBlockInNAND;
    y->param.initialiseNAND = eyaffs_initialiseNAND;
    y->param.deinitialiseNAND = eyaffs_deinitialiseNAND;

    y->param.writeChunkWithTagsToNAND = eyaffs_writeChunkWithTagsToNAND;
    y->param.readChunkWithTagsFromNAND = eyaffs_readChunkWithTagsFromNAND;
    y->param.markNANDBlockBad = eyaffs_markNANDBlockBad;
    y->param.queryNANDBlock = eyaffs_queryNANDBlock;

    y->isMounted = 0;
    y->param.removeObjectCallback = eyaffs_diriter_objremove_callback;

    if (o_skipCheckpointRead) {
        NAND_CHATTER(1, nand, "mounting with skip-checkpoint-read\n");
        y->param.skipCheckpointRead = 1;
    }
...
#if !defined(CYGSEM_FS_YAFFS_COMPATIBILITY_ECOSPRO) || (CYGSEM_FS_YAFFS_COMPATIBILITY_ECOSPRO == 0)
        ...
        case FS_INFO_DISK_USAGE:
            {
                ...
                usage->total_blocks = y->param.endBlock - y->param.startBlock + 1;
                usage->free_blocks = y->nErasedBlocks;
                usage->block_size = y->nDataBytesPerChunk * y->param.nChunksPerBlock;
                return ENOERR;
            }
#else
            (void)y;
